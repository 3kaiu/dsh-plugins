...
// 公共核(展平/收集/flutter/对比)在 ./visual-loop-shared.mjs(与单跑版共用;
// batch 版收集器由此获得 svg 资源表支持, 修复原两版漂移)
...
for (const f of files) {
  ...
  const flat = flattenNodes(nodes, 0, 0);

  const bp = generateCodeBlueprint({ canvas, nodes: flat, styles });
  blueprints.set(name, bp);

  const items = collectSpecItems(bp.tree, bp.floatings);
...
runFlutterGolden(harnessDir, 'test/batch_test.dart', { ...process.env, BATCH_MANIFEST: manifestPath }, 900000, 'flutter batch');
...
for (const name of blueprints.keys()) {
  ...
  const { pixel, blocks } = comparePair({
    pngTruth: pT, pngFlex: pF,
    manifestTruth: JSON.parse(fs.readFileSync(manT, 'utf8')),
    manifestFlex: JSON.parse(fs.readFileSync(manF, 'utf8')),
    canvas: blueprints.get(name).canvas || {},
    diffOut: path.join(buildDir, name, 'diff.png'),
  });
  rows.push({
    name,
...
console.log('\n基线汇总:', JSON.stringify({
  fixtures: okRows.length,
  skippedPureBackground: skipped.length,
  meanDiffRatio: mean(okRows.map(r => r.diffRatio)),
  maxDiffRatio: okRows.length ? round2(Math.max(...okRows.map(r => r.diffRatio))) : null,
  meanBlockMatch: mean(okRows.map(r => r.blockMatch)),
  meanPosSim: mean(okRows.map(r => r.posSim)),
  meanColorSim: mean(okRows.map(r => r.colorSim)),
}));
