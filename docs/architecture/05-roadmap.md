...
> 状态:**已执行(Phase 1-4 全部完成,里程碑 M0.5-M5 达成;实测见 11 篇 §1-19)**。
...
| M3 | matrix ≥3 组合 CI 自动跑;gate 拦截一次假完成 | ✅ 达成(11 篇 §9/§15) |
| M4 | guarded auto-merge 0 误合并 | ✅ 达成(11 篇 §15:mock 1 放行 + 3 拦截,测试 28 项) |
| M5 | 无人值守场景端到端演示通过 | ✅ 达成(11 篇 §16:00:00 失败 → 02:40 修复合入,门禁通过) |

## 6. 成功指标

| 指标 | 目标 | 现状(2026-08-17) |
| --- | --- | --- |
| **Console 日活** | = 你自己每天打开(定性:早上看早报、晚上看维护报告) | 待你每天打开(报告 CLI 已就绪:11 篇 §19) |
...
| auto-merge 误合并(FP) | 0 | ✅ M4 达成:mock 1 放行 3 拦截 + 真实 PR #6 放行(11 篇 §15/§18) |
| budget 超限率 | 0 | ✅ attempts=3 拦截实测(§15 deny-attempts) |
| needs-human 升级率 | 20~40%(过低说明闸门太松) | 运行统计中(需真实维护周期数据) |
| 闭环时长(failure → PR) | 中位数 ≤ 2 天 | 演示闭环 <1h(11 篇 §16);真实周期待统计 |
| fixture 月增量 | ≥1 | ✅ fixtures 已落(incidents×2 + autopilot.yml,11 篇 §13) |
| 兼容分(doctor) | 稳定 ≥90 | 未实现(后续项) |
