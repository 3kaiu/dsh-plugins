...
export function generateDesignSystem(blueprint: any) {
  ...
  const tokens = {
    colors: palette.slice(0, 8).map((c: any, i: any) => ({ name: `color-${i}`, value: c.hex || c, count: c.count || 1 })),
    typography: (Object.entries(profile) as [string, any][]).slice(0, 6).map(([path, spec]) => ({ name: path.replace(/[^a-z0-9]/gi,'-'), ...spec })),
    spacing: [...new Set(collectSpacings(tree))].sort((a: any, b: any) =>a-b).slice(0,8),
  }

  // components：按 role 聚类（header/card/button）
  const byRole: Record<string, any> = {}
  function collectByRole(nodes: any) {
