...
const u1 = stateUpdate({ remainingDifferences: diffs, scores: { current: { total: 0.82 } } }, { statePath: p, historyNote: 'with diffs' });
...
const u2 = stateUpdate({ resolvedDifferences: [{ path: 'header', prop: 'height', iteration: 1 }], scores: { current: { total: 0.85 } } }, { statePath: p });
